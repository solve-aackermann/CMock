...
class CMockHelperParser_cmocka
  
  attr_accessor :c_types
  
  def initialize(config)
    @config = config
    @fallback = @config.plugins.include?(:array) ? 'UNITTEST_ASSERT_EQUAL_MEMORY_ARRAY' : 'UNITTEST_ASSERT_EQUAL_MEMORY'
  ...
  def get_helper(ctype)
    ...
    return ['UNITTEST_ASSERT_EQUAL_PTR', ''] if (ctype =~ /cmock_\w+_ptr\d+/)
  ...
  def map_C_types
    ...
    @config.treat_as.each_pair do |ctype, expecttype|
      ...
      if (expecttype =~ /\*/)
        c_types[c_type] = "UNITTEST_ASSERT_EQUAL_#{expecttype.gsub(/\*/,'')}_ARRAY"
      else
        c_types[c_type] = "UNITTEST_ASSERT_EQUAL_#{expecttype}"
        c_types[c_type+'*'] ||= "UNITTEST_ASSERT_EQUAL_#{expecttype}_ARRAY"
      end
    end
    c_types
  end
  ...
  def import_source
    ...
    match_regex = Regexp.new('^\s*#define\s+(UNITTEST_ASSERT_EQUAL_(\w+))\s*\(' + Array.new(4,'\s*\w+\s*').join(',') + '\)')
    ...
    match_regex = Regexp.new('^\s*#define\s+(UNITTEST_ASSERT_EQUAL_(\w+_ARRAY))\s*\(' + Array.new(5,'\s*\w+\s*').join(',') + '\)')
    pairs = source.scan(match_regex).flatten.compact
    (pairs.size/2).times do |i|
      expect = pairs[i*2]
      ctype = pairs[(i*2)+1]
