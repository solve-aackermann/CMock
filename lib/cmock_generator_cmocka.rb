...
class CMockGenerator_cmocka

  attr_accessor :config, :file_writer, :module_name, :sourcedir, :clean_mock_name, :mock_name, :utils, :plugins, :weak, :ordered
  ...
  def create_mock(module_name, sourcedir, parsed_stuff)
  ...
  def create_mock_subdir()
    @file_writer.create_subdir(@subdir, @sourcedir)
  end

  def create_mock_header_file(parsed_stuff)
    @file_writer.create_file(@mock_name + ".h", @subdir, @sourcedir) do |file, filename|
  ...
  def create_mock_source_file(parsed_stuff)
    @file_writer.create_file(@mock_name + ".c", @subdir, @sourcedir) do |file, filename|
      create_source_header_section(file, filename, parsed_stuff[:functions])
      create_instance_structure(file, parsed_stuff[:functions])
      create_mock_init_function(file, parsed_stuff[:functions])
  ...
  def create_mock_header_header(file, filename)
    ...
    file << "#pragma GCC diagnostic ignored \"-Wint-to-pointer-cast\"\n"
    file << "#endif\n"
    file << "\n"
  end
  ...
  def create_instance_structure(file, functions)
    ...
    file << "} Mock;\n\n"
    functions.each do |function|
      file << @plugins.run(:static_datafields, function)
    end
    file << "\n\n"
  end

  def create_mock_init_function(file, functions)
    file << "void #{@clean_mock_name}_Init(void)\n{\n"
    file << "  #{@clean_mock_name}_Destroy();\n"
    functions.each do |function|
      file << "  Mock.#{function[:name]}_CallInstance = malloc(sizeof(*Mock.#{function[:name]}_CallInstance));\n"
  ...
  def create_mock_destroy_function(file, functions)
    file << "void #{@clean_mock_name}_Destroy(void)\n{\n"
    functions.each do |function|
      file << "  if(Mock.#{function[:name]}_CallInstance)\n"
      file << "    free(Mock.#{function[:name]}_CallInstance);\n"
      file << "  Mock.#{function[:name]}_CallInstance = 0;\n"
    end
